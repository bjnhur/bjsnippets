...
class ReminderEmailButton(webapp.RequestHandler):
	def get(self):
		self.response.out.write('Now start to send email to team members...')
...
class OneReminderEmail(webapp.RequestHandler):
    def post(self):
        mail.send_mail(sender="snippets <snippets@bjsnippets.appspotmail.com>", # FIX ME
...
class OneDigestEmail(webapp.RequestHandler):
    ...
    def post(self):
        ...
        if body:
            self.__send_mail(user.email, 'https://bjsnippets.appspot.com\n\n' + body) # FIX ME
        else:
